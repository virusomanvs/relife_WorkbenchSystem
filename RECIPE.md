...
| Параметр | Тип | Описание |
|----------|-----|----------|
| `needToLearn` | bool | Нужно ли изучать рецепт через предмет результата в руках перед использованием (0 - нет, 1 - да) |
...
| `showCraftWindowForResult` | bool | Показывать всплывающее окно для предмета который был скрафчен (только для крафтов где всего один результат, и крафт не производится в очереди а напрямую) (0 - нет, 1 - да) |
...
| `chance` | float | Шанс получения результата. Если у всех chance будет 0, то все. Если у хоть у одно больше 0, то будет выбран 1 согласно его весу, чем больше тем больше шанс, среди остальных  |
...
| `damageAfterCraft` | float | Урон вложению после крафта (0.0 = без урона). Должно быть отрицательным |
| `decreaseQuantityAfterCraft` | float | Уменьшение количества содержимого вложения после крафта. Число должно быть положительным, мод сам его отнимет.|
