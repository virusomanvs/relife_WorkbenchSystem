...
| `craftMode`             | `int`    | Режим крафта 0 – очередь, прямой. 1 - только прямой без возможности добавить в очередь |
| `craftTimeSec`          | `int`    | Время крафта в секундах.                               |
| `spawnOutsideWorkbench` | `bool`    | 1 – предмет появляется вне верстака, 0 – в нём.        |

📍 Позиция и ориентация результата
| Параметр                 | Тип        | Описание                              |
| ------------------------ | ---------- | ------------------------------------- |
| `resultSpawnPosOffset`   | `float[3]` | Смещение спавна относительно позиции верстака. |
| `resultSpawnPosition`    | `float[3]` | Абсолютная позиция спавна результата, если ее указать можно задать координаты где будут появляться предметы после крафта. |
| `resultSpawnOrientation` | `float[3]` | Ориентация объекта после спавна.           |

📘 Требования к изучению
| Параметр                | Тип   | Описание                                              |
| ----------------------- | ----- | ----------------------------------------------------- |
| `needToLearn`           | `int` | Нужно ли выучить рецепт (1 – да, 0 – нет). Если включить, то нельзя будет создать предмет указанный в результате, пока игрок не изучит его держа предмет в руках            |
| `deleteAfterLearn`      | `int` | Удаляется ли предмет после изучения.                   |


🎯 Дополнительные параметры мода relife_AdvSkills
| Параметр                  | Тип        | Описание                                                   |
| ------------------------- | ---------- | ---------------------------------------------------------- |
| `addPerkPointsSkillTypes` | `string[]` | Типы навыков, куда начисляются очки (например, `HUNTING`). |
| `addPerkPointsValue`      | `float[]`  | Сколько очков навыка даётся, указывается для каждого навыка выше.                               |
| `hideRecipeWithoutPerk` | `bool` | Скрывать рецепт без перка (мод relife_AdvSkills).                            |
| `requireAllPerksInList` | `bool` | Требуются все перки из списка (1) или любой один (0). |
...
| `needElectricityOn`          | `int`    | Нужно ли чтобы предмет был включен на верстаке (работает только с поддерживаемыми дополнениями).                                                             |
...
| `decreaseAttachHealth`       | `object` | Снижение прочности вложенных предметов (например, `"RLF_GrinderDiskLeft": -50.0`).                                         |
...
| `chance`                    | `float`    | Вес результата среди других результатов. Если у вас есть несколько результатов при крафте, и если указать вес выше 0 хотя бы для одного, то будет спавниться случайный результат согласно шансам. Если у всех вес появления 0, то заспавнятся все результаты крафта.                          |
| `health`                    | `float[3]` | Прочность предмета `[fixed, min, max]`. Если указать -1 у fixed, будут использоваться min, max        |
| `count`                     | `int[3]`   | Количество `[fixed, min, max]`. Если указать -1 у fixed, будут использоваться min, max .                 |
| `quantity`                  | `float[3]` | Кол-во (например, патроны) `[fixed, min, max]`. Если указать -1 у fixed, будут использоваться min, max . |
| `energy`                    | `float[3]` | Энергия `[fixed, min, max]`. Если указать -1 у fixed, будут использоваться min, max .                    |
...
| `itemPreview`            | `string`   | Превью предмета или путь к иконке edds.                                   |
| `itemCount`              | `int`      | Количество предметов.                            |
| `itemHealth`             | `int`      | Уровень здоровья предмета (например, 3 – любой кроме RUINED). |
...
| `itemQuantityAfterCraft` | `float[3]` | Изменение количества ресурса после крафта или энергии.       |
